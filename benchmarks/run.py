#!/usr/bin/env python3
"""Benchmark ieee-paper-writer vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "ieee-paper-writer" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, skill_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("skill", skill_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        skill_medians = statistics.median(
            [t["output_tokens"] for t in entry["skill"]]
        )
        savings = 1 - (skill_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "skill_median": int(skill_medians),
    ...
    avg_skill = round(statistics.mean([r["skill_median"] for r in rows]))
...
def format_table(rows, summary):
    ...
    for r in rows:
        ...
        lines.append(
            f"| {label} | {r['normal_median']} | {r['skill_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_skill']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark ieee-paper-writer vs normal Claude")
    ...
    skill_system = load_skill_system()
    ...
    results = run_benchmarks(client, args.model, prompts, skill_system, args.trials)

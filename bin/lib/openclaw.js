// ieee-paper-writer → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make ieee-paper-writer always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/ieee-paper-writer/SKILL.md into <workspace>/skills/ieee-paper-writer/
...
const SKILL_NAME = 'ieee-paper-writer';
const SKILL_VERSION = '1.0.0';
const MARK_BEGIN = '<!-- ieee-paper-writer-begin -->';
const MARK_END = '<!-- ieee-paper-writer-end -->';
...
// ieee-paper-writer SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'ieee-paper-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/ieee-paper-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    '## Caveman mode (always on)',
    '',
    'This session activates the ieee-paper-writer pipeline: Author → Editorial → IEEE Style → Reviewer.',
    '',
    "The full pipeline ruleset lives in this workspace's ieee-paper-writer skill:",
    '',
    '  skills/ieee-paper-writer/SKILL.md',
    '',
    'Use /ieee-paper-writer to run the full four-agent pipeline.',
    ,
    '',
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'ieee-paper-writer', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (!skillBody) {
    log.warn('  openclaw install requires the ieee-paper-writer repo on disk (skills/ieee-paper-writer/SKILL.md missing).');
    log.note('  Re-run from a clone or via `npx -y github:igorantunes30/author-IA-article -- --only openclaw`.');
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (ieee-paper-writer bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains ieee-paper-writer bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip ieee-paper-writer block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped ieee-paper-writer block from ${soulFile}`);
        touched = true;
      }
    }
  }

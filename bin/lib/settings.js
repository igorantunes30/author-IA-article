// ieee-paper-writer — JSONC-tolerant settings.json read/write + defensive hook validation.
//
// Lifted in spirit from gsd-build/get-shit-done's stripJsonComments + readSettings.
// Reused by bin/install.js and (optionally) by hooks/ieee-paper-activate.js so a
...
function readSettings(p) {
  ...
  try { raw = fs.readFileSync(p, 'utf8'); }
    ...
    process.stderr.write(`ieee-paper-writer: cannot read ${p}: ${e.message}\n`);
    return null;
  }
  if (!raw.trim()) return {};
  ...
  try { return JSON.parse(stripJsonComments(raw)); }
    ...
    process.stderr.write(`ieee-paper-writer: warning — ${p} is not valid JSON or JSONC: ${e.message}\n`);
    return null;
  }
}
...
// ── Idempotency probe ──────────────────────────────────────────────────────
function hasIeeePaperWriterHook(settings, event, marker = 'ieee-paper-writer') {
...
function removeIeeePaperWriterHooks(settings, marker = 'ieee-paper-writer') {
...
// on user-authored hooks that just happen to mention "ieee-paper-writer".
...
// ieee-paper-*.js) to the Claude Code plugin disables/renames those local
...
// …ieee-paper-activate.js` (issue #471). rewriteLegacyManagedHookCommands can't
...
function pruneOrphanedManagedHooks(settings, configDir) {
  ...
  // `my-ieee-paper-activate.js` is never touched) and that resolved path is absent.

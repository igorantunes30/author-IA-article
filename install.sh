#!/usr/bin/env bash
# ieee-paper-writer — installer shim.
#
...
#   curl -fsSL https://raw.githubusercontent.com/igorantunes30/author-IA-article/main/install.sh | bash -s -- --all
#
# Local clone:
...
REPO="igorantunes30/author-IA-article"

# Require Node ≥18. nvm is a common path; print a hint if missing.
if ! command -v node >/dev/null 2>&1; then
  echo "ieee-paper-writer: Node.js (≥18) required. Install:" >&2
...
if [ "$NODE_MAJOR" -lt 18 ]; then
  echo "ieee-paper-writer: Node $NODE_MAJOR too old. Need Node ≥18." >&2
...
if ! command -v npx >/dev/null 2>&1; then
  echo "ieee-paper-writer: npx required (ships with Node ≥18). Reinstall Node.js." >&2
  exit 1
fi

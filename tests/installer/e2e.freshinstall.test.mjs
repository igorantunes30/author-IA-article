...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ieee-paper-freshinstall-'));
}
...
const STATUSLINE_FILE = process.platform === 'win32'
  ? 'ieee-paper-statusline.ps1'
  : 'ieee-paper-statusline.sh';
...
function ieeePaperHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'ieee-paper-activate.js')),     'ieee-paper-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'ieee-paper-tracker.js')), 'ieee-paper-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'ieee-paper-config.js')),       'ieee-paper-config.js missing');
    ...
    assert.ok(SETTINGS.hasIeeePaperWriterHook(settings, 'SessionStart', 'ieee-paper-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasIeeePaperWriterHook(settings, 'UserPromptSubmit', 'ieee-paper-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /ieee-paper-statusline/,
      'statusLine command does not reference ieee-paper-statusline');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = ieeePaperHookCommands(settings, 'SessionStart', 'ieee-paper-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart ieee-paper-writer hook, got ${sessStart.length}`);

    const ups = ieeePaperHookCommands(settings, 'UserPromptSubmit', 'ieee-paper-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit ieee-paper-writer hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips ieee-paper-writer hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['ieee-paper-activate.js', 'ieee-paper-tracker.js', 'ieee-paper-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining ieee-paper-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /ieee-paper-activate|ieee-paper-tracker/, `${ev} still has a hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = ieeePaperHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at ieee-paper-writer should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /ieee-paper-statusline/,
      'ieee-paper-writer statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    assert.ok(SETTINGS.hasIeeePaperWriterHook(parsed, 'SessionStart', 'ieee-paper-activate'));
    assert.ok(SETTINGS.hasIeeePaperWriterHook(parsed, 'UserPromptSubmit', 'ieee-paper-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'ieee-paper-writer', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'ieee-paper-writer', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- ieee-paper-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- ieee-paper-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /IEEE-PAPER-WRITER ACTIVE/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'ieee-paper-writer', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- ieee-paper-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    ...
    assert.match(soulRaw, /<!-- ieee-paper-begin -->/, 'ieee-paper-writer block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'ieee-paper-writer')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- ieee-paper-begin -->/, 'ieee-paper-writer block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- ieee-paper-end -->/, 'ieee-paper end marker survived uninstall');
...
test('ieee-paper-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'ieee-paper-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `ieee-paper-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'ieee-paper-writer', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /IEEE-PAPER-WRITER ACTIVE/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/ieee-paper-activate.js"',
      marker: 'ieee-paper-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/ieee-paper-activate.js"',
      marker: 'ieee-paper-activate',
    });

...
// Claude Code accepts. Copying agents/ieee-paper-*.md verbatim into
// ~/.config/opencode/agents/ broke opencode startup with:
//   Configuration is invalid at .../ieee-paper-author.md
...
const SHIPPED_AGENT_FILES = ['ieee-paper-author.md', 'ieee-paper-editorial.md', 'ieee-paper-style.md', 'ieee-paper-reviewer.md'];
...
test('all shipped ieee-paper agent files become opencode-safe after transform', () => {
  for (const f of SHIPPED_AGENT_FILES) {
    ...
    assert.match(fm, /^name: ieee-paper-/m, `${f}: name field preserved`);
